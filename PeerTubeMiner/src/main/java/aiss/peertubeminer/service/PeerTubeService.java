...
public class PeerTubeService {
    ...
    public Channel getChannel(String channelId, int maxVideos, int maxComments) {
        try {
            ...
            if (ptChannel == null) throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Canal no encontrado en PeerTube");
            ...
            if (ptVideosRaw != null) {
                for (Map<String, Object> vRaw : ptVideosRaw) {
                    Video v = new Video();
                    v.setId(vRaw.get("uuid") != null ? vRaw.get("uuid").toString() : "id-desconocido");
                    v.setName(vRaw.get("name") != null ? vRaw.get("name").toString() : "Sin nombre");
                    v.setReleaseTime(vRaw.get("publishedAt") != null ? vRaw.get("publishedAt").toString() : "Sin fecha");
                    videos.add(v);
                }
            }
            ...
            System.err.println("Error de PeerTubeService: " + e.getMessage());
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Error al contactar con la API de PeerTube");
        }
    }
}